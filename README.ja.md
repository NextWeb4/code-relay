...
Node 組み込みのテストランナーを使用し、インポートの対応付け、メールボックス分離、コード抽出、Provider 解析、ポーリング、保存、HTTP ルート、GitHub リクエストの保護、資格情報のマスキング、一括更新、古い誤コードの除去を検証します。テストから実サービスや GitHub API を呼び出してはいけません。
...
- Parser、Provider、ポーリング、保存、GitHub、ブラウザの境界を変更する前に[アーキテクチャガイド](docs/architecture.md)を確認し、対応する `tests/*.test.js` に焦点を絞ったテストを追加してください。
